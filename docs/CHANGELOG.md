...
- `generate-cold-start.mjs` (new) — regenerates the 12-puzzle offline bundle `sudoku_next/app/lib/cold-start-puzzles.ts` (3 per DB tier, remapped very_easy→easy, easy→medium, medium→hard, hard→extreme).
- `generate-sample-puzzles.mjs` (new) — regenerates root `puzzles.json` (15 puzzles, 5 each easy/medium/hard) for the superseded `web/` prototype only.
- `load-supabase.mjs` (new) — streams all 148,206 puzzles into the live Supabase `puzzles` table via PostgREST (`on_conflict=puzzle`, `resolution=ignore-duplicates`, batches of 500), using a service-role key supplied at runtime.
**Deleted:** `scripts/generate-library.mjs`, `upload_to_supabase.py`, `sudoku_10000.json`, `sudoku_lower.json` (this repo); `sudoku_next/supabase/puzzles_library.sql` (the committed 10k SQL — the online pool now loads via the streaming script instead).
**Why:** Owner decisions: (1) online pool = ALL 148,206, loaded via a streaming script rather than a committed SQL file; (2) purge is a hard reset (`truncate ... restart identity cascade`, pre-launch, also clears `solves`); (3) daily split 37/146/145/37; (4) purge the legacy pipeline files too, not just add new ones. Load mechanism = streaming PostgREST loader (Option A, chosen over committing a giant SQL dump). Hard-tier daily pick = gentlest (lowest ER) for approachability.
**Operational note:** the live Supabase reload is a manual operator step (needs the service-role key, not present in the app's `.env.local`) — procedure in `sudoku_next/docs/RELOAD-RUNBOOK.md`. Migration `0010_interleave_daily.sql` is superseded by `seed.sql`'s interleaved positions and must not be re-run post-load. App code and Supabase schema/migrations were not changed.
**Files:** `scripts/{generate-seed,generate-cold-start,generate-sample-puzzles,load-supabase}.mjs` + `scripts/tests/*.test.mjs`; deleted `scripts/generate-library.mjs`, `upload_to_supabase.py`. Design + plan under `docs/superpowers/specs/2026-07-03-replace-all-puzzles-150k-daily-365-design.md` and `docs/superpowers/plans/2026-07-04-replace-all-puzzles-150k-daily-365.md`.
