 ...
 *
 * ### Timeout budget
 * Hard puzzles can be slow. We use SOLVE_TIMEOUT_MS * 4 as the container timeout
 * (configurable; generous for Colima 2-CPU VM).
 */
 ...
 * - Timeout: SOLVE_TIMEOUT_MS * 4 (generous for heavy puzzles on 2-CPU Colima VM).
 */
export async function rate(puzzles: string[]): Promise<Rating[]> {
  ...
  const raw = await runContainer(args, { timeoutMs: SOLVE_TIMEOUT_MS * 4 });
